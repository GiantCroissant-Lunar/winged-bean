...
describe("PM2 Workflow", () => {
  ...
  test("should write logs to correct locations", async () => {
    ...
    const ptyLogPath = path.join(
      __dirname,
      "../../pty-service/logs/pty-service-out.log",
    );
    const docsLogPath = path.join(
      __dirname,
      "../../sites/docs/logs/docs-site-out.log",
    );

    // Check if log files exist
    const ptyLogExists = await fs
      .access(ptyLogPath)
      .then(() => true)
      .catch(() => false);
    const docsLogExists = await fs
      .access(docsLogPath)
      .then(() => true)
      .catch(() => false);
